...
def __cf_reduce_16_64(V_i, j, W=None):
    ...
    high, low = divmod(
        (r_l_12 + V_i[4] + T_j_rotate_left[j]) & 0xFFFFFFFF, BIT_EACH_32[25])
    SS1 = high + low * BIT_EACH_32[7]
    SS2 = SS1 ^ r_l_12
    # FF
    TT1 = (((V_i[0] & V_i[1]) | (V_i[0] & V_i[2]) | (V_i[1] & V_i[2])
            ) + V_i[3] + SS2 + (W[j] ^ W[j + 4])) & 0xFFFFFFFF
    # GG
    TT2 = (((V_i[4] & V_i[5]) | ((~ V_i[4]) & V_i[6])) +
           V_i[7] + SS1 + W[j]) & 0xFFFFFFFF
...
def __cf_reduce_0_16(V_i, j, W=None):
    ...
    SS2 = SS1 ^ r_l_12
    # FF
    TT1 = ((V_i[0] ^ V_i[1] ^ V_i[2]) + V_i[3] +
           SS2 + (W[j] ^ W[j + 4])) & 0xFFFFFFFF
    # GG
...
def CF(V_i, B_i):
    ...
    for j in range(16, 68):
        ...
        W.append(X ^ r_l_15 ^ r_l_23 ^ (high_W13_7 +
                                        low_W13_7 * BIT_EACH_32[7]) ^ W[- 6])
    ...
    for j in range(0, 16):
        ...
        r_l_12 = high_A12 + low_A12 * BIT_EACH_32[12]
        high, low = divmod(
            (r_l_12 + E + T_j_rotate_left[j]) & 0xFFFFFFFF, BIT_EACH_32[25])
        SS1 = high + low * BIT_EACH_32[7]
        SS2 = SS1 ^ r_l_12
...
def CF2(V_i, B_i):
    ...
    for j in range(16, 68):
        ...
        W.append(P_1(W[- 16] ^ W[- 9] ^ (high_W3_15 + low_W3_15 * BIT_EACH_32[15])) ^ (
            high_W13_7 + low_W13_7 * BIT_EACH_32[7]) ^ W[- 6])
    V_i_1 = reduce(lambda a, b: __cf_reduce_0_16(
        a, b, W), range(0, 16), V_i.copy())
    V_i_2 = reduce(lambda a, b: __cf_reduce_16_64(
        a, b, W), range(16, 64), V_i_1)
